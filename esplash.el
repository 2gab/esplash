               ...
               (dolist (item (if (listp content) content (list content)))
                 (let* ((is-action (and (listp item) (= (safe-length item) 3) (stringp (nth 0 item))))
                        (line-str (cond (is-action (nth 0 item))
                                        ...
                                        (t item)))
                        (action (when is-action (nth 2 item)))
                   ...
                   (insert line-str)
                   (when action
                     (put-text-property item-start (point) 'esplash-action action))
                   (insert "\n"))))
      (goto-char (point-min))
...
(defun esplash-next-item ()
  "Move cursor to next actionable menu item."
  (interactive)
  (let ((orig (point)))
    (forward-line 1)
    (while (and (not (eobp))
                (not (get-text-property (point) 'esplash-action)))
      (forward-line 1))
    (when (eobp)
      (goto-char orig))))

(defun esplash-prev-item ()
  "Move cursor to previous actionable menu item."
  (interactive)
  (let ((orig (point)))
    (forward-line -1)
    (while (and (not (bobp))
                (not (get-text-property (point) 'esplash-action)))
      (forward-line -1))
    (when (and (bobp) (not (get-text-property (point) 'esplash-action)))
      (goto-char orig))))

(defun esplash-activate ()
  "Execute the action of the menu item at point."
  (interactive)
  (let ((action (get-text-property (point) 'esplash-action)))
    (when action
      (kill-buffer (current-buffer))
      (funcall action))))

(defun esplash-goto-first-item ()
  "Position cursor on the first actionable menu item."
  (goto-char (point-min))
  (unless (get-text-property (point-min) 'esplash-action)
    (let ((pos (next-single-property-change (point-min) 'esplash-action)))
...
(defun esplash ()
  ...
  (let ((esplash-buffer (get-buffer-create "*esplash*"))
    ...
    (with-current-buffer esplash-buffer
      ...
      (local-set-key (kbd "<up>") #'esplash-prev-item)
      (local-set-key (kbd "<down>") #'esplash-next-item)
      (local-set-key (kbd "k") #'esplash-prev-item)
      (local-set-key (kbd "j") #'esplash-next-item)
      (local-set-key (kbd "<return>") #'esplash-activate)
      (dolist (item esplash-body)
        (when (and (listp item) (= (safe-length item) 3) (stringp (nth 0 item)))
          (let ((key (nth 1 item))
                (act (nth 2 item)))
