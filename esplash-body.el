...
(defun esplash-body-3 ()
  (list
   (list (concat (propertize "Recently opened files             " 'face esplash-body-face)
                 (propertize "f" 'face esplash-body-key-face))
         "f"
         (lambda () (if (fboundp 'recentf-open-files)
                        (recentf-open-files)
                      (call-interactively #'find-file))))
   (list (concat (propertize "New file                          " 'face esplash-body-face)
                 (propertize "n" 'face esplash-body-key-face))
         "n"
         (lambda () (call-interactively #'find-file)))
   (list (concat (propertize "Configuration                     " 'face esplash-body-face)
                 (propertize "c" 'face esplash-body-key-face))
         "c"
         (lambda () (find-file user-init-file)))
   (list (concat (propertize "Open last session                 " 'face esplash-body-face)
                 (propertize "l" 'face esplash-body-key-face))
         "l"
         (lambda () (if (fboundp 'desktop-read)
                        (desktop-read)
                      (message "No session manager available"))))
   (list (concat (propertize "Quit                              " 'face esplash-body-face)
                 (propertize "q" 'face esplash-body-key-face))
         "q"
